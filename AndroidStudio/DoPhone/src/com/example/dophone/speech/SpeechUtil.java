package com.example.dophone.speech;
...
public class SpeechUtil {
	...
	private SearchView mResultText;
	private View btn;
	...
	public SpeechUtil(Context context, SearchView mResultText, View btn) {
		...
		initLayout();
		// 初始化识别无UI识别对象
		SpeechUtility.createUtility(context, SpeechConstant.APPID +"=575c047a");
	...
	private void initLayout() {
		btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// mResultText.setQuery("", false);// 清空显示内容
				mIatResults.clear();
				// 设置参数
				setParam();
	...
	private void printResult(RecognizerResult results) {
		...
		mResultText.setQuery(mResultText.getQuery() + resultBuffer.toString(),
				true);

		// mResultText.setSelection(mResultText.getQuery().length());
	}
	...
	/*
	 * @Override protected void onDestroy() { super.onDestroy(); // 退出时释放连接
	 * mIat.cancel(); mIat.destroy(); }
	 */
